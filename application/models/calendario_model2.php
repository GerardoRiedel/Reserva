...
//->where('hora > "2018-01-17"')
        ...
        FOREACH ($query1 as $item){
            ...
            $hora  = $this->db->select('h.id,h.hora,h.prestador')
                        ->from('hora h')
 ->join('horas_eliminadas e','e.idHora=h.id','left')
                        ->where('h.ciudad',$item->ciudad)
                        ->where('h.hora',$item->hora)
                        ->where('h.prestador',$item->prestador)
